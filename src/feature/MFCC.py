...
# Для генерації вікна Хеммінга з n точок як numpy масив
def hamming(n):
    return 0.54 - 0.46 * cos(2 * pi / n * (arange(n) + 0.5))


# Клас для роботи і керування MFCC-параметрами
class MFCCExtractor(object):
	...
	# Для виведення в консоль
    def dprint(self, msg):
        if self.verbose:
            print(msg)

	# Отримати MFCC коефіцієнти сигналу у форматі числового масиву
	...
	# Повертаємо числовий масив який є фільтр-пакетом МЕЛ-коефіцієнтів
	...
	# Повертаємо числовий масив який є DCT-матрицею n-порядку
	@staticmethod
    def dctmtx(n):
